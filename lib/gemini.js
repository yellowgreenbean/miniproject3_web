import { COACH_CATEGORIES, MAX_CHOICE_OPTIONS } from "@/lib/coachTopics";
...
function normalizeChoices(raw, planItems) {
  ...
  if (!COACH_CATEGORIES.includes(raw.category)) return null;
...
export async function getCoachReply({ history, message, context }) {
  ...
  const planItems = Array.isArray(parsed.planItems)
